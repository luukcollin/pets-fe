...
export class PetsState{
    @Selector()
    static allPets({availablePets, pendingPets, soldPets}: PetsStateModel): Pet[] {
    ...
    onSetPendingPets({patchState}: StateContext<PetsStateModel>, {pets}: PetsActions.SetPendingPets) {
        patchState({pendingPets :pets});
    }

    @Action(PetsActions.SetSoldPets)
    onSetSoldPets({patchState}: StateContext<PetsStateModel>, {pets}: PetsActions.SetSoldPets) {
        patchState({soldPets: pets});
    }

    @Action(PetsActions.SetAvailablePets)
    onSetAvailablePets({patchState}: StateContext<PetsStateModel>, {pets}: PetsActions.SetAvailablePets) {
        patchState({availablePets: pets});
    }

    @Action(PetsActions.UpdateFilterQ)
    onUpdateFilterQ({patchState}: StateContext<PetsStateModel>, {filterQ}: PetsActions.UpdateFilterQ){
        patchState({filterQ})
    }
}
